...
def call_openrouter_with_system(messages: list) -> str:
    """
    Send a conversation (messages list) with system instructions to OpenRouter.
    This function expects the messages list to already have a system message at index 0.
    ...
    payload = {
        ...
        "model": MODEL_NAME,
        # Include the entire conversation history including the system message at the start.
        # The system message (at index 0) dictates the AI's persona and constraints.
        "messages": messages,
    }
...
if __name__ == "__main__":
    ...
    # Each message is a dictionary with "role" (system/user/assistant) and "content" (the text).
    messages = []

    # Create a system message that defines the AI's persona and behavior constraints.
    # This system message is placed at index 0 (the very beginning) of the messages list.
    # Everything the AI says will be influenced by these instructions.
    system_message = {
        "role": "system",
        # The "content" field contains the actual system instructions that tell the AI how to behave.
        # We are telling the AI to act as a helpful, friendly expert assistant.
        "content": (
            "You are a helpful, friendly, and knowledgeable expert assistant. "
    ...
    # Append the system message to the beginning of the messages list.
    # The system message must always be at index 0 before any user or assistant messages.
    messages.append(system_message)

    # Print instructions for the user to understand how to interact with the chatbot.
    print("Chat with System Instructions! Type 'exit' or 'quit' to end the conversation.")
    print("The AI is following a specific persona defined by system instructions.\n")
    ...
    while True:
        ...
        # Append the user's message to the messages list with the proper structure.
        # This keeps the full conversation history for context after the system message.
        messages.append({"role": "user", "content": user_input})

        # Try to send the conversation to OpenRouter and get a response.
        try:
            # Call the OpenRouter API with the full message history including system instructions.
            assistant_response = call_openrouter_with_system(messages)

            # Print the assistant's response so the user can see it.
            print(f"Assistant: {assistant_response}\n")

            # Append the assistant's response to the messages list to maintain history.
            # This way, the next API call will have context from this response too,
            # and the system instructions will continue to influence the AI's behavior.
